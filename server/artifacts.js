...
\`\`\`sh
sim_vehicle.py -v ${settings.vehicle || "ArduCopter"} -f ${settings.physicsBackend === "json" ? `JSON:${settings.jsonHost || "127.0.0.1"}` : settings.frame || "quad"} --add-param-file ardupilot/${safeName}.param
\`\`\`
...
public:
  void Load(gazebo::physics::WorldPtr, sdf::ElementPtr sdf) override
  {
    ...
    std::cout << "[UAV Lab] Wind profile loaded mean=" << mean << " max=" << max << " variance=" << variance << std::endl;
  }
};
...
public:
  void Load(gazebo::physics::WorldPtr, sdf::ElementPtr sdf) override
  {
    ...
    std::cout << "[UAV Lab] Sensor scenario loaded enabled=" << enabled
              ...
              << " rangefinderNoiseM=" << rangefinderNoiseM << std::endl;
  }
};
